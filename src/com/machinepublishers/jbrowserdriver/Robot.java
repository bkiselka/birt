...
class Robot {
  ...
  private final AtomicReference<com.sun.glass.ui.GlassRobot> robot = new AtomicReference<com.sun.glass.ui.GlassRobot>();
        ...
        if (image == null && SettingsManager.isMonocle()) {
          try {
            ...
            final WritableImage pixels = robot.get().getScreenCapture(
            	null,
                Math.rint(stage.getX() + scene.getX()),
                Math.rint(stage.getY() + scene.getY()),
                Math.rint(scene.getWidth()),
                Math.rint(scene.getHeight()),
                false);
